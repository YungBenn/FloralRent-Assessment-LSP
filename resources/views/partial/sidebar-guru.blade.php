<div class="sidebar-guru sidebar">
    <div class="sidebar-guru-profile sidebar-profile">
        <img src="../../asset/logoCircle.svg" alt="guruTernak image">
    </div>
    <nav class="sidebar-guru-nav sidebar-nav mt-5">
        <a href="/guruternak/inbox">
            <img src="../../asset/speedometer.svg" alt="inbox">
        </a>
        <a href="/guruternak/myclass">
            <img src="../../asset/myclass.svg" alt="myclass">
            
        </a>
        <a href="/guruternak/addclass">
            Karangan Bunga
            {{-- <img src="../../asset/addclass.png" alt="addclass"> --}}
        </a>
        <a href="/guruternak/addkategori">
            Kategori
            {{-- <img src="../../asset/addclass.png" alt="addkategori"> --}}
        </a>
        <a href="/guruternak/addpenyewaan">
            Penyewaan
            {{-- <img src="../../asset/addclass.png" alt="addpenyewaan"> --}}
        </a>
        <a href="/guruternak/pengembalian">
            Pengembalian
            {{-- <img src="../../asset/addclass.png" alt="addpenyewaan"> --}}
        </a>
    </nav>
</div>